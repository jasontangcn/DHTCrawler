...
public class Utils {
    ...
    // TODO: the transaction id generated by this algorithm contains control characters
    ...
    // get the ideal bucket index of this node.
    public static int getLog2(byte[] nodeId) {
        int length = nodeId.length * 8; // length === 160
        Bitmap bitmap = new Bitmap(nodeId);
        ...
        // length === 160
        // if i = 0, result should be 159, range is [2^159, 2^160)
        // if i = 1, result should be 158, range is [2^158, 2^159)
        // ...
        // if i = 159, result should be 0, range is [2^0, 2^1).


        System.out.println("[Utils] getLog2 of <" + bitmap + ">, index = " + (length - 1 - i));
    ...
    public static byte[] getDistance(String nodeId1, String nodeId2) {
        // XOR
        byte[] n1Bytes = Utils.stringToBytes(nodeId1);
        byte[] n2Bytes = Utils.stringToBytes(nodeId2);
    ...
    public static List<Node> parseCompactNodes(String nodes) throws IOException {
        ...
        if(nodes != null && nodes.length() > 0) {
            byte[] bytes = nodes.getBytes(Charset.forName("ISO-8859-1")); // TODO: bytes.length == nodes.length()? should I indicate a Charset?
            // every node = 20 bytes(node id) + 4 bytes(IP) + 2 bytes(port) = 26 bytes
    ...
    public static String createCompactNodesString(List<Node> nodes) {
    ...
    public static List<String> createCompactPeerStringList(List<InetSocketAddress> addresses) {
        ...
        for(int i = 0; i < addresses.size(); i++) {
            ...
            // InetAddress -> ONLY address(no port)
            ...
            System.arraycopy(addrBytes, 0, peerBytes, 0, 4);
    ...
    public static List<InetSocketAddress> parseCompactPeers(List<String> peers) throws IOException {
        ...
        for(String peer : peers) {
            byte[] bytes = peer.getBytes(Charset.forName("ISO-8859-1"));
            if(bytes.length == 6) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(getLog2("41db13a5dc37f2bce0e3".getBytes(Charset.forName("ISO-8859-1"))));
    }
}
